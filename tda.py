...
class tda():
    ...
    def process_tensile_test(force, elongation, preview=False):
        ...
        """
        init_pos = tda.get_init_point(force, wnd_half_width=41)
        break_point = tda.get_break_point(force, force_drop_thr=0.25)
        limits = tda.get_slopes_limits(force,
                                       elongation, init_pos,
                                       slope_thr=0.4,
                                       slope_wnd=40)

        values = tda.get_slopes_values(force, elongation, init_pos, limits)
